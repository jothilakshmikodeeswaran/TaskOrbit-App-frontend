...
function TaskPage() {
   ...
   const [assignee, setAssignee] = useState("");
    ...
    useEffect(() => {
        //fetchTasks();
        fetchProjects();
        //fetchEmployees(); // Assuming you have an /employees endpoint
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        const token = JSON.parse(localStorage.getItem("social-app-token"));
        const data = { title, description, status, priority, project, assignee, deadline, comments };
        ...
        try {
            if (editId) {
                ...
                await backendClient.post("/tasks", data, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            resetForm();
    ...
    return (
        <main>
            ...
            <form onSubmit={handleSubmit} className="max-w-xl mx-auto mt-8 space-y-4">
                ...
                <select value={priority} onChange={(e) => setPriority(e.target.value)} className="w-full px-4 py-2 border rounded">
                ...
                <select value={assignee} onChange={(e) => setAssignee(e.target.value)} className="w-full px-4 py-2 border rounded">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {tasks.map((task) => (
                    <div key={task._id} className="border p-4 rounded shadow">
                        ...
                        <p>Project: {projects.find(p => p._id === task.project)?.name || "N/A"}</p>
                        {/* <p>Assignee: {employees.find(e => e._id === task.assignee)?.name || "Unassigned"}</p> */}
                        <p>Comments: {task.comments}</p>
                        <div className="mt-2 flex gap-2">
                            <button onClick={() => handleEdit(task)} className="bg-yellow-400 px-3 py-1 rounded">✏️ Edit</button>
                            <button onClick={() => handleDelete(task._id)} className="bg-red-500 px-3 py-1 text-white rounded">🗑️ Delete</button>
                        </div>
                    </div>
                ))}
            </section>
